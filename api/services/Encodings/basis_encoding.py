...
class BasisEncoding():
    ...
    def basis_encode_number(cls, number, n_integralbits, n_fractional_part):
        ...
        fractional_part = cls.get_fractional_part(number - int(number), n_fractional_part)
    ...
    def basis_encode_list(cls, list, n_integralbits, n_fractional_part):
        ...
        :return: List of Strings that represents the basis encoding;
                 Leading 0/1 for positive/negative number, followed by the integral and the fractional part
        """
        list = [cls.basis_encode_number(i, n_integralbits, n_fractional_part) for i in list]
        return list
    ...
    def get_integral_part(cls, integralPart, precision):
        ...
        """
        integralString = '0' * precision if integralPart == 0 else '{0:b}'.format(abs(integralPart))
    ...
    def basis_encode_number_subcircuit(cls, number, n_integralbits, n_fractional_part):
        ...
        """
        numberString = cls.basis_encode_number(number, n_integralbits, n_fractional_part)
        numberArray = cls.convert_bitstring_to_intarray(numberString)

        n_qubits = 1 + n_integralbits + n_fractional_part

        encoding_subcircuit = QuantumCircuit(n_qubits, name='basisencode {}'.format(number))
        [(encoding_subcircuit.x(i) if numberArray[i] == 1 else 0) for i in range(n_qubits)]
    ...
    def basis_encode_list_subcircuit(cls, list, n_integralbits, n_fractional_part):
        ...
        numberArray = [cls.convert_bitstring_to_intarray(listAsString[i]) for i in range(len(listAsString))]
        ...
        for j in range(0, len(numberArray)):
            [(encoding_subcircuit.x(i + (j * len(numberArray[j])))
              if numberArray[j][i] == 1 else 0) for i in range(len(numberArray[j]))]
        #encoding_subcircuit.draw(output='mpl')
        #plt.show()
